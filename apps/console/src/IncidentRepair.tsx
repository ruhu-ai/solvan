...
export function VerificationPanel({ incident }: { incident: Incident }): React.JSX.Element {
  ...
  return <section><div className="verification-header"><div><p className="eyebrow">Independent recovery oracle</p><h2>{verification.id} · mitigation verification</h2><p>{verification.profile} · owner {verification.owner}</p></div><StatusBadge label={presentation.label} tone={presentation.tone} machine={verification.verdict} /></div><div className="verification-summary"><LabelValue label="Resolved binding" value={verification.binding} /><LabelValue label="Immutable threshold" value={verification.threshold} /><LabelValue label="Fresh synthetic probe" value={verification.synthetic} /></div><div className="interval-rail" aria-hidden="true">{verification.intervals.map((interval, index) => <div className={`interval interval-${index}`} key={interval.name}><span>{interval.name}</span></div>)}</div><div className="responsive-table card"><table><caption>Equivalent table for baseline and verification intervals</caption><thead><tr><th>Interval</th><th>Window</th><th>Error ratio</th><th>p95 latency</th><th>Result</th></tr></thead><tbody>{verification.intervals.map((interval) => <tr key={interval.name}><td>{interval.name}</td><td>{interval.window}</td><td>{interval.error_ratio}</td><td>{interval.p95}</td><td>{interval.result}</td></tr>)}</tbody></table></div><div className="security-note"><span aria-hidden="true"><ShieldCheck size={18} strokeWidth={1.75} /></span><div><strong>{presentation.headline}</strong><p>{presentation.detail}</p></div></div></section>;
}
