    ...
    <div className="alert-report-column">
      ...
      <article className="card alert-answer-first" aria-label="Alert investigation report">
        {report.sections.sort((a, b) => a.ordinal - b.ordinal).map((section) => <section key={section.section_id} className="alert-report-section">
          ...
          {section.claims.length > 0 ? section.claims.map((claim) => <div key={claim.claim_template_id} className="alert-claim"><p>{String(claim.typed_values.sentence ?? "Committed alert fact")}</p><div className="alert-claim-meta"><span>{sourceStatusLabel(claim.source_status_kind)}</span>{claim.citation_refs.map((citation) => <button type="button" className="source-chip" key={citation} title={citation}>Source · {shortId(citation)}<span className="sr-only">{` — full reference ${citation}`}</span></button>)}</div></div>) : <p className="alert-holding">{holdingCopy(section.section_id, section.status)}</p>}</div>
        </section>)}
      </article>
