...
| architecture enforcement | verified locally | machine rules, actionable failures and negative structural tests; twelve dated size exceptions recorded in `config/architecture-exceptions.yaml`, including five release-delivery exceptions expiring 2026-08-20 | complete the named deployed qualification runs, split every expiring boundary, and repeat in CI |
...
| cloud release | implemented, not verified | shared Terraform stack with isolated dev/staging configs, Cloud Build, deployment helpers, exact topology/preflight evaluator, GCS hash/binding-validating console projection, and release-evidence gate exist; no staging deployment receipt exists | staging preflight and S1–S6 receipts pass |
