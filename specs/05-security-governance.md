...
| unconnected, substituted, or wrong-region release source | human-authorized regional GitHub App connection, time-bounded Google-required bootstrap role removed at `COMPLETE`, `COMPLETE` connection preflight, Terraform-owned linked repository, exact resource-name/remote-URI verification, no source token in release inputs | `CCR-008` |
| interrupted or capacity-exhausted Agent Runtime deployment produces duplicate institutional agents | deployment/commit/material labels, per-agent durable checkpoint, exact remote reconciliation, monotonic hydration of provider-populated output timestamps, refuse replacement while an interrupted create is unresolved, and whole-fleet regional capacity admission before the first create | `PR-031`, `PR-033` |
