"""Coordinator client for the private Deployment Controller service."""
...
class DeploymentControllerConfiguration:
    base_url: str
    audience: str
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError("Deployment Controller endpoint must be one HTTPS origin")
...
class DeploymentControllerClient:
    def __init__(
        self,
        *,
        config: DeploymentControllerConfiguration,
        client: httpx.Client,
    ...
    def execute_private_command(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if response.status_code >= 400:
            raise RuntimeError(f"Deployment Controller returned HTTP {response.status_code}")
        value = response.json()
        if not isinstance(value, dict):
            raise RuntimeError("Deployment Controller returned a non-object response")
        return value
